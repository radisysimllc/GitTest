...
For example, if you are using stats like SinglePlayerMap and MultiplayerMap to filter content, the player is only going to be in one of them at once, but both values will be available to query from the service at any time.
